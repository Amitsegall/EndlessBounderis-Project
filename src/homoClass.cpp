...
void HomoClass::update(ofVideoGrabber vid){
    
    
    
    imitate(warpedColor, vid); // importing the current image / graphics / live camera
    ...
    if(homographyReady) {
        ...
        warpPerspective(vid, warpedColor, homography, CV_INTER_LINEAR);
